  ...
  return (
    <div className="   md:w-[600px]  m-2 p-4 rounded-t-full md:rounded-tr-none  md:rounded-l-full relative shadow-xl">
      ...
      <div className="flex flex-col md:flex-row  ">
        <div className="flex flex-row justify-center items-center w-[170px] ">
          <Image
            ...
            height={200}
            alt="name"
            className="w-[170px] h-[160px] bg-slate-600 rounded-full"
          />
        </div>
      ...
      <div className="flex justify-end space-x-1">
        <Link
          href={{
            pathname: `/teamscard/teamcarddetail/${BT_ID}`,
            query: { year: year },
          }}
        >
          <button className="border-blue-400  text-blue-400 border-2  hover:text-gray-600 hover:bg-blue-400 py-1 px-2 rounded-lg ">
            View
          </button>
        </Link>
        {/* <Link href={'/teamscard/editcard/2313213'}  > */}
        <button
          className="border-green-400  border-2 text-green-400 hover:text-gray-600 hover:bg-green-400 py-1 px-2 rounded-lg "
          onClick={(e) => handleUpdateBtn(e, _id)}
        >
          Edit
        </button>
        {/* </Link> */}
        <button
          className="border-red-400  border-2 text-red-400 hover:text-gray-600  hover:bg-red-400 py-1 px-2 rounded-lg "
          onClick={(e) => handleDelete(e, name, BT_ID)}
        >
          Delete
        </button>
      </div>
...
const Addbtn = () => {
  return (
    <div className="   md:w-[600px]  m-2 p-4 rounded-t-full md:rounded-tr-none  md:rounded-l-full relative">
      <div className="">
...
export default function Teamcard({ params }) {
  ...
  const delectActionFunc = async () => {
    ...
    const newData = await allDataCard.filter(
      (item) => item.name !== deletename.name,
    );
    console.log(newData);
  ...
  return (
    <div className="">
      <div className="text-center">
        <Title name={"ScholoGamma 2023 Teams"} />
      </div>
      <Link href={"/displayteam"} className="btn bg-blue-300">
        View page
      </Link>

      {/* For Add the button */}
      <Link
        href={`/teamscard/addcard/${params.year}`}
      ...
      <ModalAlert isOpen={isDelete} onClose={() => setIsDelete(false)}>
        <div className="">
          ...
          <div className="flex flex-row justify-end space-x-4 mt-[50px]">
            <CommButton
              title={"cancel"}
              CssClasses={"bg-blue-500"}
              handdleSubmite={() => setIsDelete(false)}
            />

            <CommButton
              title={"Confrim"}
              CssClasses={"bg-red-500"}
              handdleSubmite={delectActionFunc}
              isSubmitting={isDeletting}
            />
          </div>
        </div>
      </ModalAlert>
