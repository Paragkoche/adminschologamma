...
export const POST = async (req, { params }) => {
  try {
    ...
    if (check) {
      return new Response(
        JSON.stringify({ msg: "This year teams already exist !", ok: false }),
        { status: 200 },
      );
    }
...
export const GET = async (req, { params }) => {
  try {
    ...
    if (!check) {
      return new Response(
        JSON.stringify({
          msg: "Team for " + params.year + " are not exist!",
          ok: false,
        }),
        { status: 200 },
      );
...
export const PUT = async (req, { params }) => {
  ...
  try {
    ...
    if (!checkOld) {
      return new Response(
        JSON.stringify({
          msg: "This bt teams  are not  exist!",
          type: "warning",
          ok: false,
        }),
        { status: 200 },
    ...
    if (findID) {
      return new Response(
        JSON.stringify({
          msg: "This BTID are already  exist!",
          type: "warning",
          ok: false,
        }),
        { status: 200 },
    ...
    try {
      const profile = await Teams.findOneAndUpdate(
        { _id: checkOld._id },
        { $set: { data: dataNew } },
      );
      if (!profile) {
      ...
      return new Response(
        JSON.stringify({
          msg: "Your data has been added sucessfully",
          data: profile,
          type: "success",
          ok: true,
        }),
