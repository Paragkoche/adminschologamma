  ...
  return (
    <div className="flex flex-col justify-center items-center">
      <div className="w-[270px] h-[270px] rounded-full bg-red-400 flex justify-center items-center">
        {committeeData.photo !== null && (
          <Image
            src={committeeData.photo}
            width={250}
            height={240}
            className="w-[250px] h-[240px] rounded-full bg-gray-100"
          ></Image>
        )}
      </div>
      <div className="flex flex-row justify-center items-center">
        <h1 className="font-semibold text-[22px] shadow-md text-orange-400">
          {committeeData.name}
        </h1>
        {/* <Link href={'/committee/update/34324'}  > */}
        <button onClick={() => setIsUpdate(true)}>
          <FiEdit className="p-2 bg-cyan-800 text-white text-[30px] shadow-lg rounded-full hover:bg-cyan-900" />
        </button>
        {/* </Link>  */}
      </div>
      ...
      <div className="">
        <Title
          name={`${committeeData.name} Committee Events`}
          classes={"text-center"}
        ></Title>
        <Modal isOpen={isUpdate} onClose={() => setIsUpdate(false)}>
          <UpdateCommittee
            datacommittee={committeeData}
            setIsUpdate={setIsUpdate}
            setDataCommittee={setCommitteeData}
          />
        </Modal>
        <div className="flex flex-wrap justify-center items-center space-x-[20px]">
          {eventData.length == 0 && (
            <h2 className="text-center font-semibold bg-blue-400">
              Comming Soon...
            </h2>
          )}
          {eventData?.map((item) => (
          ...
          <Modal2 isOpen={isUpdate1} onClose={() => setIsUpdate1(false)}>
            <UpdateEvent
              updateEventData={updateCardData}
              setIsUpdate={setIsUpdate1}
              committeeID={committeeData._id}
              setAllEventData={setEventData}
            />
          </Modal2>
        </div>
      </div>
      ...
      <div className="">
        ...
        <div className="flex flex-row justify-around items-center flex-wrap w-[100%]">
          {teamData
            ?.filter((item) => item.position == "Co-Head")
            .map((item) => (
              <ProfileClientCard key={item._id} {...item} />
            ))}
        </div>
