...
function AddCommittee() {
  ...
  const handdleSubmit = async (e) => {
    ...
    if (!isValidUsername(committeeData.username)) {
      return dd.setAlertFunc(
        "error",
        "Invalid username. Must be 8-20 alphanumeric characters and unique.",
      );
    }
    ...
    if (committeeData.password !== committeeData.cpassword) {
      return dd.setAlertFunc(
        "error",
        "Password are not match with the confirmed password",
      );
    }
    ...
    const res = await fetch("/api/committee/add", {
      ...
      body: JSON.stringify({
        ...committeeData,
        photo: imgUrl.result.secure_url,
      }),
      headers: {
