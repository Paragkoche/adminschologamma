      ...
      <Link href={`/committee/view/${_id}`} className={`${css ? css : ""}`}>
        <div className="w-[250px] h-[250px] rounded-full gol shadow-2xl platinumcss flex justify-center items-center">
          <Image
            ...
            height={210}
            alt={name}
            className="w-[210px] h-[210px] rounded-full bg-gray-600 "
          ></Image>
        </div>
      </Link>

      <div className=" bg-red-700 text-white font-extrabold  shadow-xl flex justify-center items-center w-[300px] px-[4px] text-[22px] rounded-full py-[5px]">
        <h1>{name}</h1>
      </div>
    </div>
  );
...
const Committe = () => {
  ...
  return (
    <div>
      <Title
        name={"All Committies in Forum"}
        classes={"text-blue-400 text-center"}
      ></Title>
