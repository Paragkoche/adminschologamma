...
const YearCard = ({ year }) => {
  return (
    <Link
      href={`/teamscard/${year}`}
      className="text-[44px] font-extrabold drop-shadow-2xl text-orange-400  w-[150px] h-[150px] rounded-lg shadow-lg flex flex-wrap  justify-center items-center relative  cursor-pointer"
    >
...
const Teams = () => {
  ...
  const OnSubmitYear = async (e) => {
    ...
    if (data.ok === true) {
      setYearList((pre) => [
        ...pre,
        { year: data?.data.year, _id: data?.data._id },
      ]);
    }
  ...
  return (
    <div>
      ...
      <label
        className="text-[84px] font-extrabold drop-shadow-2xl text-green-400  w-[150px] h-[150px] rounded-lg shadow-lg flex flex-wrap  justify-center items-center cursor-pointer  hover:bg-green-400 hover:text-slate-800 "
        htmlFor="my_modal_6"
        onClick={OnSubmitYear}
      >
        +
      </label>
